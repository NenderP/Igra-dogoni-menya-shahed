...
> Статус: черновик для обсуждения. Владелец: @ox-alpha, ревьюер: @kinder-joy228.
> Меняем только через договорённость в WAR_ROOM.md.
...
## Открытые вопросы (обсуждаем тут же, не тянем)

1. Аккаунты v1 без пароля (player_id = личность) — ок? Или сразу токен+пароль?
2. Кто генерирует награды за бой (`game_over.rewards`) — сервер боя пишет в профиль игрока модуля гачи?
3. `state_sync` шлём целиком при каждом действии или только дельты? (предлагаю: целиком в v0, дельты позже)
4. Формат колоды для ботов: `/gacha` отдаёт готовую колоду по запросу `get_bot_deck {difficulty}`?
